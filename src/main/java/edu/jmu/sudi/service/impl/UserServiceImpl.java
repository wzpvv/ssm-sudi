...
public class UserServiceImpl implements UserService {
    ...
    public LayuiTableDataResult findRoleListByUserId(Long userId) {
        //查询所有的角色列表
        List<RoleEntity> roleList = roleMapper.findRoleListMap();
        List<Map<String,Object>> roleListMap = new ArrayList<>();
        //根据用户ID查询其拥有的角色
        List<Long> roleByUserId = userMapper.findRoleByUserId(userId);
        for (RoleEntity roleEntity : roleList) {
            Map<String,Object> stringObjectMap = new HashMap<>();
            stringObjectMap.put("roleId", roleEntity.getRoleId());
            stringObjectMap.put("roleName", roleEntity.getRoleName());
            stringObjectMap.put("lastModifyBy", roleEntity.getLastModifyBy());
            stringObjectMap.put("lastModifyTime", roleEntity.getLastModifyTime());
            stringObjectMap.put(SystemConstant.LAY_CHECK, false);
            roleListMap.add(stringObjectMap);
            for (Long roleId : roleByUserId) {
                //遍历角色编号列表，标记用户拥有的角色
                if (stringObjectMap.get("roleId").equals(roleId)){
                    stringObjectMap.put(SystemConstant.LAY_CHECK, true);
                    break;
                }
            }
        }
